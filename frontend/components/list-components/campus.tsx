...
const CampusComponent: NextPage<CampusComponentProps> = ({ campus }) => {
  ...
  const busySlots: { start: moment.Moment; end: moment.Moment }[] =
    campus.schedules.map((schedule) => {
  ...
  if (sorttedBusySlots.length === 0) {
    ...
    if (workEnd.isAfter(sorttedBusySlots[sorttedBusySlots.length - 1].end)) {
      availableSlots.push({
        start: sorttedBusySlots[sorttedBusySlots.length - 1].end,
        end: workEnd,
      });
    }
  }
  ...
  let busyIdx = 0,
    availableIdx = 0;
  while (
    busyIdx < sorttedBusySlots.length &&
    availableIdx < availableSlots.length
  ) {
  ...
  return (
    <CampusListItem>
      <CampusListItemName>
        {capitalizeFirstLetter(campus.id)}
      </CampusListItemName>
      <CampusListSubItem>
        {allSlots.map((slot, idx) => (
          <CampusListItemEvent key={idx} className={idx === 0 ? '__first' : ''}>
            <CampusListItemEventHeader
              className={slot.type === 'busy' ? 'busy' : 'available'}
            >
              {slot.type === 'busy' ? 'Busy' : 'Available'}
            </CampusListItemEventHeader>
